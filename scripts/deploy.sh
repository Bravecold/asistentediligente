...
archive_path="$(mktemp /tmp/asistente-api.XXXXXX.zip)"
...
az storage blob service-properties update --account-name "$storage_name" --static-website --index-document index.html --404-document index.html --auth-mode login --output none
az storage blob upload-batch --account-name "$storage_name" --auth-mode login --destination '$web' --source frontend --overwrite --output none
rm -f "$archive_path" frontend/config.js
